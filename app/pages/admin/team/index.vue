...
const loadMembers = async () => {
  ...
  try {
    members.value = await getCompanyMembers(companyId)
  } finally {
...
const handleRemove = async (relationshipId: string, isPending: boolean) => {
  ...
  const confirmText = isPending
    ? '¿Cancelar esta invitación pendiente?'
    : '¿Eliminar a este miembro de la empresa? Perderá el acceso de inmediato.'
  ...
  try {
    ...
    flashSuccess(isPending ? 'Invitación cancelada.' : 'Miembro removido.')
    await loadMembers()
  } finally {
    ...
    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500 max-w-xl">
          Administra qué personas tienen acceso a
          <span class="font-medium text-slate-700">{{ selectedCompany?.name ?? 'esta empresa' }}</span>,
          su rol e invitaciones pendientes de respuesta.
        </p>
      </div>
